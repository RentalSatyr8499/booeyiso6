...
for (var i = 3; i < 5; i++) {
    u = Math.random() < 0.5 ? -1 : 1;
    if (i == 3){
        movingStats = initializeGhost(partyGuestImages[i], partyGuests[i], booeyX + 50, getRandomInt(400, 250), 0, u);
    } else if (i==4) {
        movingStats = initializeGhost(partyGuestImages[i], partyGuests[i], booeyX - 50, getRandomInt(400, 250), 0, u);
    } else {
        movingStats = initializeGhost(partyGuestImages[i], partyGuests[i], booeyX, getRandomInt(400, 250), 0, u);
    }
    movingStats["currentImg"] = partyGuestImages[i][0];
    partyGuestMovingStats.push(movingStats);
}
